...
Registry payload는 특정 governance 상태 시점에서 활성화된 전체 Authority Record 스냅샷을 의미한다.

Registry payload는 특정 version 또는 snapshot identifier에 대응하는 전체 ACTIVE / REVOKED / RETIRED records의 정렬된 집합이며, 부분 registry 조각은 payload로 간주되지 않는다.
...
public_key MUST be base64-encoded raw key bytes as defined in the corresponding crypto profile.

Implementations MUST validate key length and format according to the declared signature_algorithm profile.

scope SHALL be interpreted as scope_contract_ids representing Contract-ID level authority scope.
...
추가 알고리즘은 Governance Council 승인 및 헌법 개정 절차를 통해서만 확장될 수 있다.
...
Valid Signature Predicate

유효한 서명은 다음 조건을 동시에 만족해야 한다.

(status == ACTIVE) AND
(now ≥ effective_at_utc) AND
(revoked_at_utc == null OR now < revoked_at_utc)

조건 위반 시 MUST FAIL_CLOSED.
...
Registry payload는 FULL snapshot이어야 하며 partial payload는 허용되지 않는다.

Governance Evidence는 다음을 참조해야 한다.

registry_sha256

registry_version 또는 snapshot identifier
...
AUTHORITY_NOT_FOUND → authority_id 또는 actor_id가 registry에 존재하지 않음

AUTHORITY_REVOKED → status == REVOKED 또는 now ≥ revoked_at_utc

INVALID_TIER → authority_tier가 taxonomy에 없음 또는 요구 tier 미만

INVALID_SIGNATURE_ALGORITHM → 지원되지 않는 알고리즘

REGISTRY_HASH_MISMATCH → registry hash 불일치
...
Authority Registry는 다음 스펙과 동일 Registry를 공유해야 한다.
...
모든 cross-contract 검증 시 authority_registry_sha256 값이 동일해야 한다.

불일치 시 MUST FAIL_CLOSED.
...
SUPERLOCK 선언 시 Authority Registry payload 자체도 registry_sha256에 바인딩된 LOCK 또는 SUPERLOCK declaration을 가져야 한다.
