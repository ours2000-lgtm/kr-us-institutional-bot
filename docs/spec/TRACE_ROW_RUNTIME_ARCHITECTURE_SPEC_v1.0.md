...
docs/spec/TRACE_ROW_RUNTIME_ARCHITECTURE_SPEC_v1.0.md
...
This specification defines the governance runtime architecture, control flow, decision ordering, and invariant guarantees ensuring deterministic, fail-closed operation across evaluation, promotion, and risk enforcement.
...
The runtime SHALL enforce the following invariant guarantees:
...
Risk Engine outputs are the single source of truth; Dashboard implementations MUST NOT recalculate risk locally.
...
Risk Engine outputs remain the single source of truth for risk scoring and level determination.
...
Dashboard SHALL display runtime evidence metadata including registry_snapshot_id, policy_profile_id, and evaluation identifiers to ensure traceability.

Dashboard MUST clearly indicate degraded or simulation states when present in runtime evidence.
...
YES — affects decision correctness and promotion safety.