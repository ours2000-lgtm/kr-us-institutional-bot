...
RiskEngine V1.9 — Final Polished V1 Line (Stateless + Typed + Ops-Friendly)

특징:
- Stateless 이벤트 버퍼 (멀티스레드/멀티코루틴 안전)
- InputSnapshot TypedDict 도입 (타입 안정성 + IDE 지원)
...
- meta: received_at / processed_at / fallback / risk_level / risk_score / latency_ms
- 예외 메타: exception_type (정상: None, fallback: 예외 타입)
...
class RiskEngineV19:
    ...
    - emit_detail_events 플래그로 이벤트 양 조절
    """

    def __init__(
        self,
        engine_version: str = "V1.9",
        market: str = "KR",
        *,
    ...
    def _validate_input(self, snapshot: InputSnapshot, events: List[Event]) -> None:
        ...
        - volume은 V1.9에서 optional (future use)
        - 간단한 타입 및 범위 체크 포함
        """
        ...
        # volume은 현재 score 계산에 직접 사용하지 않지만,
        # 향후 Orderflow/Liquidity 모델과 연동될 수 있으므로 존재만 체크.
    ...
    def _compute_risk_score(
        ...
        liquidity = float(snapshot.get("liquidity", 1.0))
    ...
    def _classify_exception(self, exc: Exception) -> Tuple[RiskLevel, bool]:
        ...
        - ValueError: 입력 데이터 문제 → medium / halt=False
        - TODO: UpstreamTimeoutError, ConfigError 등 추가 가능
        - 기타: 시스템 문제 → high / halt=True
        """
    ...
    def _build_fallback_result(
        ...
        *,
        ...
        - _classify_exception() 결과에 따라 risk_level / halt_trading 결정
        """
        exc_event = self._append_event(
            ...
            meta={
                ...
                # "exception_stack": traceback.format_exc(),  # 필요 시 활성화
            },
        )
        ...
        return {
            ...
            "meta": {
                ...
                "risk_score": risk_score,
                "exception_type": exc.__class__.__name__,
                # "exception_stack": traceback.format_exc(),  # 필요 시 사용
            },
        }
...
if __name__ == "__main__":
    ...
    snapshot_ok: InputSnapshot = {
        ...
        "volatility": 0.02,
        "liquidity": 0.9,
        "timestamp": RiskEngineV19._now_iso(),
        "raw": {},
    }
