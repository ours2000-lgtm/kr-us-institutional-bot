...
def _as_violation(layer: Layer, code: str, message: str, *, event_index: int = -1, context: Dict[str, Any] | None = None) -> InvariantViolation:
...
def validate_trace(
    ...
    *,
    ...
    # - STRICT_CRITICAL: CRITICAL만 있으면 ok=False (WARNING만 있으면 ok=True)
    ...
    try:
        ...
        v = _as_violation(Layer.GENESIS, "G_CANONICALIZE_EXCEPTION", f"canonicalize raised exception: {type(e).__name__}: {e}")
        decision = policy.decide([v])
        ok = _compute_ok([v], decision.fail_closed, ok_mode)
        return ValidationResult(
            ok=ok,
    ...
    try:
        ...
        l1 = [_as_violation(Layer.L1_SHAPE, "L1_EXCEPTION", f"Layer1 raised exception: {type(e).__name__}: {e}")]
    ...
    try:
        ...
        # signature mismatch guard (makes the error actionable)
        l2 = [_as_violation(Layer.L2_TRANSITION, "L2_SIGNATURE_MISMATCH", f"Layer2 signature mismatch: {e}")]
    except Exception as e:
        l2 = [_as_violation(Layer.L2_TRANSITION, "L2_EXCEPTION", f"Layer2 raised exception: {type(e).__name__}: {e}")]
    ...
    try:
        ...
        l3 = [_as_violation(Layer.L3_SEMANTIC, "L3_EXCEPTION", f"Layer3 raised exception: {type(e).__name__}: {e}")]
