# ======================================================================
# run_us_v8_plus.py — 미국장 V8 PLUS 자동매매 메인 엔진 (MASTER BUILD)
# ======================================================================
...
def initialize_system():
    ...
    config = load_config(CONFIG_DIR, "US")

    safe_log("====================================================")
    safe_log("   🇺🇸 미국 자동매매 V8 PLUS ENGINE — START")
    safe_log("====================================================")

    # 브로커 연결
    broker = AlpacaBrokerV8(config)
    ...
    safe_log("[SYSTEM] 미국 V8 PLUS 초기화 완료")
...
def main_loop():
    ...
    schedule = config["SCHEDULE"]["US"]

    # 시장 시작 전 대기
    safe_log("[SCHEDULE] 시장 시작 전 대기 중…")
    wait_until(schedule["start"])
    safe_log("[SCHEDULE] 미국장 자동매매 시작")
